...
public:
    ...
    vec3 shade(const Ray& ray, vec3 point, vec3 n, Light light) {
...
public:
    ...
    vec3 trace(const Ray& ray, float tMin, float tMax) {
        ...
        return (hitSurface != nullptr) ? hitSurface->shade(ray, hitPoint, hitSurface->getNormal(hitPoint), *lights[0]) : vec3(0.0f, 0.0f, 0.0f); //if null -> black color
    }
};
...
void render() {
    ...
    Light light(vec3(-4.0f, 0.0f, -3.0f), vec3(1.0f, 1.0f, 1.0f));
